...
export class InitialSeeder extends BaseSeeder {
    ...
    async run(dataSource: DataSource): Promise<void> {
        ...
        try {
            ...
            // 创建索引
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_record_user_id" ON "buildingai_mind_map"."mind_map_ai_chat_record" ("user_id")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_record_mind_map_id" ON "buildingai_mind_map"."mind_map_ai_chat_record" ("mind_map_id")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_record_status" ON "buildingai_mind_map"."mind_map_ai_chat_record" ("status")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_record_is_deleted" ON "buildingai_mind_map"."mind_map_ai_chat_record" ("is_deleted")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_record_is_deleted_created_at" ON "buildingai_mind_map"."mind_map_ai_chat_record" ("is_deleted", "created_at")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_record_user_id_created_at" ON "buildingai_mind_map"."mind_map_ai_chat_record" ("user_id", "created_at")`,
            );
            ...
            // 创建索引，使用有意义的名称而不是自动生成
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_message_model_id" ON "buildingai_mind_map"."mind_map_ai_chat_message" ("model_id")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_message_role" ON "buildingai_mind_map"."mind_map_ai_chat_message" ("role")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_message_sequence" ON "buildingai_mind_map"."mind_map_ai_chat_message" ("sequence")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_message_role_created_at" ON "buildingai_mind_map"."mind_map_ai_chat_message" ("role", "created_at")`,
            );
            await queryRunner.query(
                `CREATE INDEX IF NOT EXISTS "idx_mind_map_chat_message_conversation_id_created_at" ON "buildingai_mind_map"."mind_map_ai_chat_message" ("conversation_id", "created_at")`,
            );
            ...
            const foreignKeys = await queryRunner.query(`
                ...
                WHERE table_schema = 'buildingai_mind_map' 
                AND table_name = 'mind_map_ai_chat_record' 
                AND constraint_type = 'FOREIGN KEY'
                AND (constraint_name LIKE '%user_id%' OR constraint_name LIKE '%FK_%')
            `);

            if (foreignKeys.length === 0) {
                await queryRunner.query(
                    `ALTER TABLE "buildingai_mind_map"."mind_map_ai_chat_record" ADD FOREIGN KEY ("user_id") REFERENCES "user"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
                );
            }

            const foreignKeys2 = await queryRunner.query(`
                SELECT constraint_name 
                FROM information_schema.table_constraints 
                WHERE table_schema = 'buildingai_mind_map' 
                AND table_name = 'mind_map_ai_chat_record' 
                AND constraint_type = 'FOREIGN KEY'
                AND (constraint_name LIKE '%mind_map_id%' OR constraint_name LIKE '%FK_%')
            `);

            if (foreignKeys2.length === 0) {
                await queryRunner.query(
                    `ALTER TABLE "buildingai_mind_map"."mind_map_ai_chat_record" ADD FOREIGN KEY ("mind_map_id") REFERENCES "buildingai_mind_map"."mind_map_record"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
                );
            }

            const foreignKeys3 = await queryRunner.query(`
                SELECT constraint_name 
                FROM information_schema.table_constraints 
                WHERE table_schema = 'buildingai_mind_map' 
                AND table_name = 'mind_map_ai_chat_message' 
                AND constraint_type = 'FOREIGN KEY'
                AND (constraint_name LIKE '%conversation_id%' OR constraint_name LIKE '%FK_%')
            `);

            if (foreignKeys3.length === 0) {
                await queryRunner.query(
                    `ALTER TABLE "buildingai_mind_map"."mind_map_ai_chat_message" ADD FOREIGN KEY ("conversation_id") REFERENCES "buildingai_mind_map"."mind_map_ai_chat_record"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
                );
            }

            const foreignKeys4 = await queryRunner.query(`
                SELECT constraint_name 
                FROM information_schema.table_constraints 
                WHERE table_schema = 'buildingai_mind_map' 
                AND table_name = 'mind_map_ai_chat_message' 
                AND constraint_type = 'FOREIGN KEY'
                AND (constraint_name LIKE '%model_id%' OR constraint_name LIKE '%FK_%')
            `);

            if (foreignKeys4.length === 0) {
                await queryRunner.query(
                    `ALTER TABLE "buildingai_mind_map"."mind_map_ai_chat_message" ADD FOREIGN KEY ("model_id") REFERENCES "ai_models"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
                );
            }
